...
use crate::{AttributeValue, AttributeValue2, MailboxDatum};
...
pub(crate) fn msg_att_gmail_labels2(input: &[u8]) -> IResult<&[u8], AttributeValue2> {
    map(gmail_label_list, |_| AttributeValue2::Ignored)(input)
}
...
pub(crate) fn msg_att_gmail_msgid2(input: &[u8]) -> IResult<&[u8], AttributeValue2> {
    map(gmail_msgid, |_| AttributeValue2::Ignored)(input)
}
