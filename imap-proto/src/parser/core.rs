...
pub fn string2(input: &[u8]) -> IResult<&[u8], StringRet> {
    alt((
        map(quoted, |data| (Some(data), None)),
        map(literal2, |size| (None, Some(size))),
    ))(input)
}
...
pub fn literal2(input: &[u8]) -> IResult<&[u8], usize> {
    map(delimited(tag(b"{"), number, tag(b"}")), |n| n as usize)(input)
}
...
pub type StringRet<'a> = (Option<&'a [u8]>, Option<usize>);

pub fn nstring2(input: &[u8]) -> IResult<&[u8], StringRet> {
    alt((map(nil, |_| (None, None)), string2))(input)
}
