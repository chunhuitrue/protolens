...
pub(crate) fn msg_att_mod_seq2(input: &[u8]) -> IResult<&[u8], AttributeValue2> {
    let (i, (_, _num)) = tuple((tag_no_case("MODSEQ "), paren_delimited(number_64)))(input)?;
    Ok((i, AttributeValue2::Ignored))
}
